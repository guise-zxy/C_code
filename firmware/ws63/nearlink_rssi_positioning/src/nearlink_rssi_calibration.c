...
#include "pinctrl.h"
#include "soc_osal.h"
#include "nearlink_rssi_calibration.h"

#define NEARLINK_RSSI_CAL_RSSI_MIN (-100)
#define NEARLINK_RSSI_CAL_RSSI_MAX (-20)
#define NEARLINK_RSSI_CAL_RAW_MIN (-127)
#define NEARLINK_RSSI_CAL_RAW_MAX 20
#define NEARLINK_RSSI_CAL_MAD_MAX 30U
 ...
 */
#define NEARLINK_RSSI_CAL_LOG "[nearlink rssi cal]"
#define NEARLINK_RSSI_CAL_BUTTON_PIN 13
#define NEARLINK_RSSI_CAL_LED_PIN 5
#define NEARLINK_RSSI_CAL_LED_PIN_MODE 4
#define NEARLINK_RSSI_CAL_BUTTON_POLL_MS 50
#define NEARLINK_RSSI_CAL_BUTTON_DEBOUNCE 3
#define NEARLINK_RSSI_CAL_LONG_PRESS_COUNT 40
#define NEARLINK_RSSI_CAL_SAMPLE_COUNT 31
#define NEARLINK_RSSI_CAL_LED_BLINK_COUNT 5
#define NEARLINK_RSSI_CAL_LED_HOLD_COUNT 60
#define NEARLINK_RSSI_CAL_LED_BOOT_CLEAR_COUNT 10
#define NEARLINK_RSSI_CAL_TASK_PRIO 30
#define NEARLINK_RSSI_CAL_TASK_STACK_SIZE 0x800
#define NEARLINK_RSSI_CAL_NV_ID 0x5101
#define NEARLINK_RSSI_CAL_NV_MAGIC 0x52535349
#define NEARLINK_RSSI_CAL_NV_VERSION 1
#define NEARLINK_RSSI_CAL_GPIO_SET_ADDR 0x44028030
#define NEARLINK_RSSI_CAL_GPIO_CLEAR_ADDR 0x44028034
 ...
 */
typedef enum {
    NEARLINK_RSSI_LED_IDLE = 0,
    NEARLINK_RSSI_LED_RECORDING,
    NEARLINK_RSSI_LED_SUCCESS,
    NEARLINK_RSSI_LED_FAILURE,
} nearlink_rssi_led_state_t;
...
typedef struct {
    ...
    uint32_t checksum;
} nearlink_rssi_calibration_nv_t;

static volatile bool g_connected = false;
static volatile bool g_calibrating = false;
static volatile nearlink_rssi_led_state_t g_led_state = NEARLINK_RSSI_LED_IDLE;
static bool g_led_boot_clear_pending = true;
static int8_t g_calibration_samples[NEARLINK_RSSI_CAL_SAMPLE_COUNT] = {0};
static uint8_t g_calibration_count = 0;
static int8_t g_rssi_at_1m = CONFIG_NEARLINK_RSSI_POSITIONING_RSSI_AT_1M;
 ...
 * @endif
 */
static uint32_t nearlink_rssi_calibration_checksum(const nearlink_rssi_calibration_nv_t *record)
{
    /*
 ...
 * @endif
 */
static bool nearlink_rssi_calibration_record_valid(const nearlink_rssi_calibration_nv_t *record, uint16_t length)
{
    /*
     * Validate the format, physical ranges, sample policy and checksum before using A.
     * 使用 A 之前，校验记录格式、物理取值范围、采样策略和校验和。
     */
    return (length == sizeof(*record)) && (record->magic == NEARLINK_RSSI_CAL_NV_MAGIC) &&
           (record->version == NEARLINK_RSSI_CAL_NV_VERSION) && (record->rssi_at_1m >= NEARLINK_RSSI_CAL_RSSI_MIN) &&
           (record->rssi_at_1m <= NEARLINK_RSSI_CAL_RSSI_MAX) && (record->mad <= NEARLINK_RSSI_CAL_MAD_MAX) &&
           (record->min_rssi >= NEARLINK_RSSI_CAL_RAW_MIN) && (record->max_rssi <= NEARLINK_RSSI_CAL_RAW_MAX) &&
           (record->min_rssi <= record->rssi_at_1m) && (record->rssi_at_1m <= record->max_rssi) &&
           (record->sample_count == NEARLINK_RSSI_CAL_SAMPLE_COUNT) &&
           (record->checksum == nearlink_rssi_calibration_checksum(record));
}
 ...
 * @endif
 */
static void nearlink_rssi_calibration_load(void)
{
    nearlink_rssi_calibration_nv_t record = {0};
    uint16_t length = 0;
    errcode_t ret = uapi_nv_read(NEARLINK_RSSI_CAL_NV_ID, sizeof(record), &length, (uint8_t *)&record);
    if ((ret == ERRCODE_SUCC) && nearlink_rssi_calibration_record_valid(&record, length)) {
        g_rssi_at_1m = (int8_t)record.rssi_at_1m;
        osal_printk("%s NV calibration loaded: A=%d dBm, MAD=%u dB, range=[%d,%d] dBm, samples=%u\r\n",
                    NEARLINK_RSSI_CAL_LOG, record.rssi_at_1m, record.mad, record.min_rssi, record.max_rssi,
                    record.sample_count);
        return;
    }
    osal_printk("%s no valid NV calibration, use default A=%d dBm\r\n", NEARLINK_RSSI_CAL_LOG, g_rssi_at_1m);
}
 ...
 * @endif
 */
static errcode_t nearlink_rssi_calibration_save(int8_t median, uint8_t mad, int8_t min_rssi, int8_t max_rssi)
{
    nearlink_rssi_calibration_nv_t record = {0};

    record.magic = NEARLINK_RSSI_CAL_NV_MAGIC;
    record.version = NEARLINK_RSSI_CAL_NV_VERSION;
    ...
    record.sample_count = NEARLINK_RSSI_CAL_SAMPLE_COUNT;
    record.checksum = nearlink_rssi_calibration_checksum(&record);
    return uapi_nv_write(NEARLINK_RSSI_CAL_NV_ID, (const uint8_t *)&record, sizeof(record));
}
 ...
 * @endif
 */
static void nearlink_rssi_calibration_sort(int8_t *values, uint8_t count)
{
    uint8_t i;
    uint8_t j;
 ...
 * @endif
 */
static void nearlink_rssi_calibration_sort_u8(uint8_t *values, uint8_t count)
{
    uint8_t i;
    uint8_t j;
 ...
 * @endif
 */
static inline void nearlink_rssi_led_delay(uint8_t read_count)
{
    ...
    for (i = 0; i < read_count; i++) {
        uapi_reg_read32(NEARLINK_RSSI_CAL_GPIO_SET_ADDR, value);
    }
    unused(value);
}
 ...
 * @endif
 */
static inline void nearlink_rssi_led_send_one(void)
{
    uapi_reg_setbit(NEARLINK_RSSI_CAL_GPIO_SET_ADDR, NEARLINK_RSSI_CAL_LED_PIN);
    nearlink_rssi_led_delay(SK6805_ONE_HIGH_DELAY);
    uapi_reg_setbit(NEARLINK_RSSI_CAL_GPIO_CLEAR_ADDR, NEARLINK_RSSI_CAL_LED_PIN);
    nearlink_rssi_led_delay(SK6805_ONE_LOW_DELAY);
}
 ...
 * @endif
 */
static inline void nearlink_rssi_led_send_zero(void)
{
    uapi_reg_setbit(NEARLINK_RSSI_CAL_GPIO_SET_ADDR, NEARLINK_RSSI_CAL_LED_PIN);
    nearlink_rssi_led_delay(SK6805_ZERO_HIGH_DELAY);
    uapi_reg_setbit(NEARLINK_RSSI_CAL_GPIO_CLEAR_ADDR, NEARLINK_RSSI_CAL_LED_PIN);
    nearlink_rssi_led_delay(SK6805_ZERO_LOW_DELAY);
}
 ...
 * @endif
 */
static void nearlink_rssi_led_send_byte(uint8_t value)
{
    uint8_t mask;

    for (mask = 0x80U; mask != 0U; mask >>= 1U) {
        if ((value & mask) != 0U) {
            nearlink_rssi_led_send_one();
        } else {
            nearlink_rssi_led_send_zero();
        }
    }
}
 ...
 * @endif
 */
static void nearlink_rssi_led_set(uint8_t red, uint8_t green, uint8_t blue)
{
    /*
     ...
     */
    nearlink_rssi_led_send_byte(green);
    nearlink_rssi_led_send_byte(red);
    nearlink_rssi_led_send_byte(blue);
...
static void nearlink_rssi_led_service(void)
{
     ...
     */
    if (g_led_boot_clear_pending && (state == NEARLINK_RSSI_LED_IDLE) && g_connected) {
        if (++boot_clear_ticks >= NEARLINK_RSSI_CAL_LED_BOOT_CLEAR_COUNT) {
            nearlink_rssi_led_off();
            g_led_boot_clear_pending = false;
            osal_printk("%s stale LED state cleared after SLE connection\r\n", NEARLINK_RSSI_CAL_LOG);
        }
        return;
    }
    ...
    if (state != last_state) {
        ...
        state_ticks = 0;
        blink_on = false;
        if (state == NEARLINK_RSSI_LED_RECORDING) {
            blink_on = true;
            nearlink_rssi_led_set(0, 0, 0xFF);
        } else if (state == NEARLINK_RSSI_LED_SUCCESS) {
            nearlink_rssi_led_set(0, 0xFF, 0);
        } else if (state == NEARLINK_RSSI_LED_FAILURE) {
            nearlink_rssi_led_set(0xFF, 0, 0);
        } else {
            nearlink_rssi_led_off();
        }
        return;
    ...
    state_ticks++;
    if ((state == NEARLINK_RSSI_LED_RECORDING) && (state_ticks >= NEARLINK_RSSI_CAL_LED_BLINK_COUNT)) {
        state_ticks = 0;
        blink_on = !blink_on;
        nearlink_rssi_led_set(0, 0, blink_on ? 0xFF : 0);
    } else if (((state == NEARLINK_RSSI_LED_SUCCESS) || (state == NEARLINK_RSSI_LED_FAILURE)) &&
               (state_ticks >= NEARLINK_RSSI_CAL_LED_HOLD_COUNT)) {
        g_led_state = NEARLINK_RSSI_LED_IDLE;
    }
}
 ...
 * @endif
 */
static void nearlink_rssi_calibration_start(void)
{
    if (!g_connected) {
        osal_printk("%s long press ignored: SLE is not connected\r\n", NEARLINK_RSSI_CAL_LOG);
        g_led_state = NEARLINK_RSSI_LED_FAILURE;
        return;
    }
    if (g_calibrating) {
        return;
 ...
 * @endif
 */
static void *nearlink_rssi_calibration_button_task(const char *arg)
{
    bool last_raw = false;
    ...
    unused(arg);
    while (1) {
        bool raw_pressed = (uapi_gpio_get_val(NEARLINK_RSSI_CAL_BUTTON_PIN) == GPIO_LEVEL_LOW);
        /*
        ...
        if (raw_pressed == last_raw) {
            if (stable_count < NEARLINK_RSSI_CAL_BUTTON_DEBOUNCE) {
                stable_count++;
            }
        } else {
         ...
         */
        if (stable_pressed) {
            if (press_count < NEARLINK_RSSI_CAL_LONG_PRESS_COUNT) {
                press_count++;
            }
            if ((press_count >= NEARLINK_RSSI_CAL_LONG_PRESS_COUNT) && !long_press_handled) {
                long_press_handled = true;
                nearlink_rssi_calibration_start();
            }
        } else {
            press_count = 0;
            long_press_handled = false;
        }
        nearlink_rssi_led_service();
        (void)osal_msleep(NEARLINK_RSSI_CAL_BUTTON_POLL_MS);
    }
    return NULL;
}

errcode_t nearlink_rssi_calibration_init(void)
{
     ...
     */
    (void)uapi_pin_set_mode(NEARLINK_RSSI_CAL_BUTTON_PIN, HAL_PIO_FUNC_GPIO);
    (void)uapi_pin_set_pull(NEARLINK_RSSI_CAL_BUTTON_PIN, PIN_PULL_TYPE_UP);
    (void)uapi_gpio_set_dir(NEARLINK_RSSI_CAL_BUTTON_PIN, GPIO_DIRECTION_INPUT);
    /*
     * GPIO5 drives the SK6805 DIN pin directly.
     * GPIO5 直接驱动 SK6805 的 DIN 引脚。
     */
    (void)uapi_pin_set_mode(NEARLINK_RSSI_CAL_LED_PIN, NEARLINK_RSSI_CAL_LED_PIN_MODE);
    (void)uapi_gpio_set_dir(NEARLINK_RSSI_CAL_LED_PIN, GPIO_DIRECTION_OUTPUT);
    uapi_reg_setbit(NEARLINK_RSSI_CAL_GPIO_CLEAR_ADDR, NEARLINK_RSSI_CAL_LED_PIN);
    (void)osal_msleep(1);
    g_led_state = NEARLINK_RSSI_LED_IDLE;
    /*
     ...
     */
    nearlink_rssi_calibration_load();

    osal_kthread_lock();
    task_handle = osal_kthread_create((osal_kthread_handler)nearlink_rssi_calibration_button_task, 0, "NearlinkRssiCal",
                                      NEARLINK_RSSI_CAL_TASK_STACK_SIZE);
    if (task_handle != NULL) {
        osal_kthread_set_priority(task_handle, NEARLINK_RSSI_CAL_TASK_PRIO);
    }
    osal_kthread_unlock();
    if (task_handle == NULL) {
...
void nearlink_rssi_calibration_set_connected(bool connected)
{
...
bool nearlink_rssi_calibration_is_active(void)
{
    return g_calibrating;
}

bool nearlink_rssi_calibration_add_sample(int8_t rssi)
{
    int8_t sorted[NEARLINK_RSSI_CAL_SAMPLE_COUNT];
    uint8_t deviations[NEARLINK_RSSI_CAL_SAMPLE_COUNT];
    int8_t median;
    uint8_t mad;
    uint8_t i;
    errcode_t ret;

    if (!g_calibrating || (g_calibration_count >= NEARLINK_RSSI_CAL_SAMPLE_COUNT)) {
        return false;
    }
    g_calibration_samples[g_calibration_count++] = rssi;
    if (((g_calibration_count % 5U) == 0U) || (g_calibration_count == NEARLINK_RSSI_CAL_SAMPLE_COUNT)) {
        osal_printk("%s recording: %u/%u, rssi=%d dBm\r\n", NEARLINK_RSSI_CAL_LOG, g_calibration_count,
                    NEARLINK_RSSI_CAL_SAMPLE_COUNT, rssi);
    }
    if (g_calibration_count < NEARLINK_RSSI_CAL_SAMPLE_COUNT) {
        return false;
    }
     ...
     */
    for (i = 0; i < NEARLINK_RSSI_CAL_SAMPLE_COUNT; i++) {
        sorted[i] = g_calibration_samples[i];
    }
    nearlink_rssi_calibration_sort(sorted, NEARLINK_RSSI_CAL_SAMPLE_COUNT);
    median = sorted[NEARLINK_RSSI_CAL_SAMPLE_COUNT / 2U];
    /*
    ...
    nearlink_rssi_calibration_sort_u8(deviations, NEARLINK_RSSI_CAL_SAMPLE_COUNT);
    mad = deviations[NEARLINK_RSSI_CAL_SAMPLE_COUNT / 2U];
    /*
     * Publish A immediately; persist the same value for subsequent normal boots.
     * 立即启用新的 A，并将同一数值持久化供后续正常启动加载。
     */
    g_rssi_at_1m = median;
    ret = nearlink_rssi_calibration_save(median, mad, sorted[0], sorted[NEARLINK_RSSI_CAL_SAMPLE_COUNT - 1U]);
    g_calibrating = false;
    g_led_state = (ret == ERRCODE_SUCC) ? NEARLINK_RSSI_LED_SUCCESS : NEARLINK_RSSI_LED_FAILURE;
    osal_printk(
        "%s calibration complete: A=%d dBm, MAD=%u dB, range=[%d,%d] dBm, "
        "samples=%u, nv=%s\r\n",
        NEARLINK_RSSI_CAL_LOG, median, mad, sorted[0], sorted[NEARLINK_RSSI_CAL_SAMPLE_COUNT - 1U], NEARLINK_RSSI_CAL_SAMPLE_COUNT,
        (ret == ERRCODE_SUCC) ? "ok" : "failed");
    return true;
}

int8_t nearlink_rssi_calibration_get_rssi_at_1m(void)
{
    return g_rssi_at_1m;
}
