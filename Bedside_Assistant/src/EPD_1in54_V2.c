...
{
    uapi_gpio_set_val(EPD_RST_PIN, 1);
    osal_mdelay(200);
...
static void EPD_1IN54_V2_SendCommand(UBYTE Reg)
{
    ...
    uapi_gpio_set_val(EPD_CS_PIN, 1);
}
...
{
    uapi_gpio_set_val(EPD_DC_PIN, 1);
    uapi_gpio_set_val(EPD_CS_PIN, 0);
...
{
    osal_printk("e-Paper busy\r\n");
    while(uapi_gpio_get_val(EPD_BUSY_PIN) == 1) {      //LOW: idle, HIGH: busy
        osal_mdelay(10);
        // osal_printk("e-Paper busying\r\n");
    }
    osal_printk("e-Paper busy release\r\n");
}
