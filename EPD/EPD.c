...
esp_err_t epd_gpio_init(void)
{
    ...
    /*
     * BUSY 引脚：使能内部下拉（~45kΩ）。
     * EPD 深睡眠后 BUSY 变高阻态，如果没有下拉，GPIO 会读到高电平，
     * 误判为 BUSY 导致超时。
     * 正常工作时 EPD 推挽输出会覆盖下拉。
     */
...
{
    ESP_LOGI(TAG, "Turn on display (full refresh)");
...
/*
 * 快速模式：使用 0xC7 波形，比 0xF7 少一次全屏清屏，闪烁更少。
 * 需要在 init 中预先加载温度值（0x22→0x91→0x20）。
 */
static esp_err_t epd_turn_on_display_fast(void)
{
    ESP_LOGI(TAG, "Turn on display (fast refresh)");

    EPD_RETURN_ON_ERROR(epd_send_cmd(0x22));
    EPD_RETURN_ON_ERROR(epd_send_data(0xC7));
...
/*
 * 局部刷：Display Update Control 2
 * 0xFF: Waveshare V2 局部刷新使用值，闪烁更少
 * 0x20: Master Activation，开始刷新
 */
static esp_err_t epd_turn_on_display_partial(void)
{
    ESP_LOGI(TAG, "Turn on display (partial refresh)");

    EPD_RETURN_ON_ERROR(epd_send_cmd(0x22));
    EPD_RETURN_ON_ERROR(epd_send_data(0xFF));
...
esp_err_t epd_init(void)
{
    ...
    /*
     * 参考 Waveshare Python 官方代码：硬件复位后不检查 BUSY，
     * 直接发 0x12 软件复位，等 BUSY 释放后再继续初始化。
     * 深睡眠唤醒后 BUSY 可能长时间为高，固定延时更可靠。
     */
     ...
     * 0x6E: 内部温度传感器，默认偏移
     */
    EPD_RETURN_ON_ERROR(epd_send_cmd(0x1A));
    EPD_RETURN_ON_ERROR(epd_send_data(0x6E));

    /*
     * 预加载温度值到内部寄存器，fast 模式 (0xC7) 不会再次加载。
     */
    EPD_RETURN_ON_ERROR(epd_send_cmd(0x22));
    EPD_RETURN_ON_ERROR(epd_send_data(0x91));
    EPD_RETURN_ON_ERROR(epd_send_cmd(0x20));
    EPD_RETURN_ON_ERROR(epd_wait_busy_timeout(EPD_WAIT_TIMEOUT_MS));
...
/*
 * 从深睡眠中唤醒，仅做硬件复位 + 必要的寄存器初始化。
 * 不做 0x12 软件复位，否则会清空显示 RAM。
 *
 * 注意：参考 Waveshare Arduino 官方代码，硬件复位后不检查 BUSY。
 * 深睡眠后 BUSY 引脚状态不可靠，直接发命令即可。
 * BUSY 只在 Active Display (0x20) 之后才有意义。
 */
esp_err_t epd_wake_from_sleep(void)
{
    ESP_LOGI(TAG, "EPD wake from sleep");

    epd_reset();

    /*
     * 深睡眠后硬件复位，需要等待让内部振荡器稳定。
     * Arduino 官方代码复位后延迟 100ms 后直接发命令，不检查 BUSY。
     */
    vTaskDelay(pdMS_TO_TICKS(100));

    /*
     * 硬件复位后 0x1A 寄存器被重置为默认值 0x00（内部温度传感器关闭）。
     * 局部刷新时 0x22 → 0xFF 会触发 Load Temperature，
     * 如果 0x1A 未正确配置，会导致驱动电压不正确。
     */
    EPD_RETURN_ON_ERROR(epd_send_cmd(0x1A));
    EPD_RETURN_ON_ERROR(epd_send_data(0x6E));
...
/*
 * 快速显示：用 0xC7 波形代替 0xF7，跳过全屏清屏步骤，闪烁更少。
 * 需要在 epd_init() 中预加载温度值（0x22→0x91→0x20）。
 *
 * 注意：与全刷一样需要写 0x24 和 0x26 两路数据，
 * 否则红黑两路数据不一致会导致字符边缘模糊扭曲。
 */
esp_err_t epd_display_fast(const uint8_t *frame_buffer)
{
    ...
    ESP_LOGI(TAG, "EPD display fast start, bytes=%u", (unsigned int)EPD_BUF_SIZE);

    /* 0x3C: 边框波形，0x05: GS 过渡，LUT1/LUT3/LUT4 */
    EPD_RETURN_ON_ERROR(epd_send_cmd(0x3C));
    EPD_RETURN_ON_ERROR(epd_send_data(0x05));

    /* 0x21: Display Update Control, 关闭 B/W 模式 */
    EPD_RETURN_ON_ERROR(epd_send_cmd(0x21));
    EPD_RETURN_ON_ERROR(epd_send_data(0x00));
    EPD_RETURN_ON_ERROR(epd_send_data(0x00));
    ...
    /* 必须写 0x26，否则红黑两路不一致导致字迹模糊扭曲 */
    EPD_RETURN_ON_ERROR(epd_set_cursor(0, 0));
    EPD_RETURN_ON_ERROR(epd_send_cmd(0x26));
    EPD_RETURN_ON_ERROR(epd_send_data_buffer(frame_buffer, EPD_BUF_SIZE));

    EPD_RETURN_ON_ERROR(epd_turn_on_display_fast());

    ESP_LOGI(TAG, "EPD display fast done");
    return ESP_OK;
}
...
/*
 * 深睡眠：直接发送 0x07→0xA5。
 * 不发送 0x02（Power Off）也不检查 BUSY，因为全刷完成后
 * 0x02 可能导致 BUSY 长时间不释放。
 * 深睡眠后通过硬件复位唤醒。
 */
esp_err_t epd_sleep(void)
{
    ...
    /* 0x07: Deep Sleep, 0xA5: check code */
    EPD_RETURN_ON_ERROR(epd_send_cmd(0x07));
    EPD_RETURN_ON_ERROR(epd_send_data(0xA5));

    return ESP_OK;
}

/*
 * 局部刷新：只写入 0x24（黑白数据），不写 0x26（红色数据），
 * 使用局部刷新波形，减少全屏闪烁。
 * 
 * 注意：调用前必须已执行 epd_init() 唤醒屏幕。
 * 调用后建议 epd_sleep() 进入休眠。
 */
esp_err_t epd_display_partial(const uint8_t *frame_buffer)
{
    ...
    ESP_LOGI(TAG, "EPD partial display start, bytes=%u", (unsigned int)EPD_BUF_SIZE);
     ...
     */
    EPD_RETURN_ON_ERROR(epd_send_cmd(0x3C));
    EPD_RETURN_ON_ERROR(epd_send_data(0x80));

    /*
     * 0x21: Display Update Control
     * 0x00, 0x00: 关闭 B/W 模式，进入局部刷新模式
     */
    EPD_RETURN_ON_ERROR(epd_send_cmd(0x21));
    EPD_RETURN_ON_ERROR(epd_send_data(0x00));
    EPD_RETURN_ON_ERROR(epd_send_data(0x00));
    ...
    /* 只写 0x24 黑白数据，不写 0x26 红色数据 */
    EPD_RETURN_ON_ERROR(epd_send_cmd(0x24));
    EPD_RETURN_ON_ERROR(epd_send_data_buffer(frame_buffer, EPD_BUF_SIZE));

    /* 触发局部刷新 */
    EPD_RETURN_ON_ERROR(epd_turn_on_display_partial());

    ESP_LOGI(TAG, "EPD partial display done");
    return ESP_OK;
}
